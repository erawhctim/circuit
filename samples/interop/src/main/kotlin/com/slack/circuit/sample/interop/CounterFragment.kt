...
/**
 * A traditional Android Fragment implementation of a counter UI, mimicking [CounterView]. It uses
 * lightweight MVI semantics via [setState] and setters for increment/decrement.
 */
...
/**
 * An interop compose function that renders [CounterFragment] as a Circuit-powered
 * [AndroidViewBinding].
 */
@Composable
fun CounterFragmentComposable(state: CounterScreen.State, modifier: Modifier = Modifier) {
  ...
  val fragmentManager =
    (LocalContext.current as? FragmentActivity)?.supportFragmentManager ?: return
  ...
  AndroidViewBinding(
    ...
    modifier = modifier,
    update = { getFragment().apply { setState(state.count) } }
  )
}
